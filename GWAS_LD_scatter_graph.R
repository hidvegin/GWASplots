...
#
# Plot style:
#   A Remington-style grey point cloud can be shown behind the chromosome-wise
#   fitted/smoothed curves. The point cloud represents raw TASSEL pairwise r2
#   values; the fitted curves and decay tables are calculated from the selected
#   decay method.
#
# Background:
...
# Remington-style point cloud behind the fitted/smoothed LD decay curve.
# Each point is one raw TASSEL pairwise r2 value.
# Set max_raw_points_to_plot <- Inf to draw every point.
# Note: plotting all points can make SVG files very large and slow to edit.
show_raw_point_cloud <- TRUE

# Inf = draw all raw points; e.g. 100000 or 300000 = draw a reproducible subset.
max_raw_points_to_plot <- Inf

raw_point_seed <- 123

# Darker and more visible raw point cloud.
...
plot_curve <- function(curve_df, method, output_prefix, ld_points = NULL) {
  chr_levels <- natural_chr_levels(curve_df$chr)

  curve_df <- curve_df %>%
    mutate(chr = factor(chr, levels = chr_levels))

  # Remington-style raw point cloud: each point is one TASSEL marker-pair r2 value.
  # Points are sampled only for plotting, not for model fitting or table calculation.
  plot_points <- NULL
  if (show_raw_point_cloud && !is.null(ld_points) && nrow(ld_points) > 0) {
    plot_points <- ld_points %>%
      filter(
        !is.na(dist_kb),
        !is.na(r2),
        dist_kb >= 0,
    ...
    # If max_raw_points_to_plot is Inf, every raw point is retained.
    # Otherwise, draw a reproducible subset only when the filtered point cloud
    # is larger than max_raw_points_to_plot.
    if (is.finite(max_raw_points_to_plot) && nrow(plot_points) > max_raw_points_to_plot) {
      set.seed(raw_point_seed)
      plot_points <- plot_points %>%
        slice_sample(n = max_raw_points_to_plot)
    }
  }

  p <- ggplot()

  if (!is.null(plot_points) && nrow(plot_points) > 0) {
    p <- p +
      geom_point(
        data = plot_points,
        aes(x = dist_kb, y = r2),
  ...
  p <- p +
    geom_line(
      data = curve_df,
      aes(x = dist_kb, y = r2_curve, color = chr),
      linewidth = 0.75,
      na.rm = TRUE
    ) +
    scale_x_continuous(
...
for (method in methods_to_run) {
  ...
  plot_curve(curve_df, method = method, output_prefix = output_prefix, ld_points = ld)
